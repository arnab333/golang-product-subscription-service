...
func (app *Config) render(w http.ResponseWriter, r *http.Request, templateName string, td *TemplateData) {
	// this are necessary for every template we want to render. these are common for all
	partials := []string{
	...
	// these are the actual template we want to render on every "render" call
	var templateSlice []string
	templateSlice = append(templateSlice, fmt.Sprintf("%s/%s", pathToTemplates, templateName))
