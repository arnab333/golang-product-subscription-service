...
func (app *Config) SubscribeToPlan(w http.ResponseWriter, r *http.Request) {
	...
	// generate a manual - in PDF
	app.WG.Add(1)
	go func() {
		defer app.WG.Done()

		pdf := app.generateManual(user, plan)
		err := pdf.OutputFileAndClose(fmt.Sprintf("./tmp_pdf/%d_manual.pdf", user.ID))
		if err != nil {
			app.ErrorChan <- err
			return
		}
		...
		msg := Message{
			...
			AttachmentMap: map[string]string{
				"Manual.pdf": fmt.Sprintf("./tmp_pdf/%d_manual.pdf", user.ID),
			},
		}

		app.sendEmail(msg)

		// testing the app error chan
...
func (app *Config) generateManual(u data.User, plan *data.Plan) *gofpdf.Fpdf {
	...
	imprtr := gofpdi.NewImporter()

	time.Sleep(5 * time.Second)

	t := imprtr.ImportPage(pdf, "./pdf/manual.pdf", 1, "/MediaBox")
	pdf.AddPage()

	imprtr.UseImportedTemplate(pdf, t, 0, 0, 215.9, 0)
